...
class ProtocolTransporter extends BaseTransporter {
  constructor (opts) {
    ...
    this._bootstrapPort = bootstrapPort;
  ...
  async _createBootstrapNode () {
    this._bootstrapNode = dht({ ephemeral: true });

    // runs the bootstrap UDP node in a specific port
    this._bootstrapNode.listen(this._bootstrapPort);

    await pEvent(this._bootstrapNode, 'ready');

    this._bootstrapNode.on('add-node', (node) => {
      this.logger.debug('BOOTSTRAP_NODE: add node', node);
    });

    this._bootstrapNode.on('remove-node', (node) => {
      this.logger.debug('BOOTSTRAP_NODE: remove node', node);
    });
  ...
  _getBootstrapIp () {
    return publicIp.v4().catch(() => {
      this.logger.error('BOOTSTRAP_NODE: error trying to get external ip');
      return internalIp.v4();
    }).catch(err => {
      this.logger.error('BOOTSTRAP_NODE: error trying to get interal ip');
      throw err;
    });
  }
}

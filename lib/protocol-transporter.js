...
class ProtocolTransporter extends BaseTransporter {
  constructor (opts) {
    const { key, hyperswarm = {}, validate = () => true, asBootstrap = false, bootstrapPort = 0 } = opts;
  ...
  async connect () {
    ...
    this.logger.info('bootstrap nodes: ', opts.bootstrap || 'default');
  ...
  async _createBootstrapNode () {
    ...
    address.address = require('internal-ip').v4.sync();

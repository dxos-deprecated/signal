...
class ProtocolTransporter extends BaseTransporter {
  ...
  async connect () {
    ...
    this._bootstrapID = crypto.createHash('sha256')
      .update(this.broker.nodeID + 'bootstrap')
      .digest();
  ...
  async _createBootstrapNode () {
    this._bootstrapNode = dht({ ephemeral: true, adaptive: true, id: this._bootstrapID });
